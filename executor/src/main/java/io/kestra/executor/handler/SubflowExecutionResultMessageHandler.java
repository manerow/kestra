            ...
            if (execution.hasTaskRunJoinable(message.getParentTaskRun())) { // TODO if we remove this check, we can avoid adding 'iteration' on the 'isSame()' method
                try {
                    ...
                    if (task instanceof ForEachItem.ForEachItemExecutable forEachItem) {
                        ...
                        Map<String, Object> outputs = MapUtils.deepMerge(taskOutputService.getOutputs(taskRun), message.getOutputs());
                        var previousTaskRun = execution.findTaskRunByTaskRunId(taskRun.getId());
                        if (previousTaskRun == null) {
                            throw new IllegalStateException("Should never happen");
                        }
                        var previousOutputs = taskOutputService.getOutputs(previousTaskRun);
                        var pair = ExecutableUtils.manageIterations(
                            runContext.storage(),
                            taskRun,
                            outputs,
                            previousTaskRun,
                        ...
                        taskRun = pair.getLeft();
                        if (!MapUtils.isEmpty(pair.getRight())) {
                            taskOutputService.saveOutputs(taskRun, pair.getRight());
                        }
                    } else {
